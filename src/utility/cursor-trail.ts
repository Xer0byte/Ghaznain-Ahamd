...
export function cursorTrail(props: CursorTrail) {
  ...
  // Extract HSL values from the CSS variable (e.g., "183 63% 40%")
  const accentColor = `hsla(${
    colorRaw ? colorRaw.trim().split(" ").join(",") : "0, 0%, 0%"
  }, 0.35)`;
  ...
  let AnimationFeature = {
    friction: 0.45, // Reduced friction for smoother glide
    trails: 30,     // Increased trails for denser ribbon
    size: 50,       // Increased size for longer, smoother tail
    dampening: 0.25, // Slightly higher dampening for smoother stops
    tension: 0.99,  // Increased tension for more fluidity
  };
  ...
  class Line {
    ...
    constructor(e: LineProps) {
      ...
      const initialPosition = e.cursorPosition ?? { x: 0, y: 0 };
      this.nodes = [];
      for (let i = 0; i < AnimationFeature.size; i++) {
        const t = new NewNode();
        t.x = initialPosition.x;
        t.y = initialPosition.y;
        this.nodes.push(t);
      }
    }
